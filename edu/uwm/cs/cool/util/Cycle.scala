...
/**
 * Cycle computation of graph represented by vertices 1..max
 * (Not 0 .. (max-1) !).  The result returns whether the given node
 * is involved in a cycle.
 */
class Cycle(max : Int, val succ : PartialFunction[Int,Set[Int]])
  extends PartialFunction[Int,Boolean]
{
...
object TestCycle extends Application {
  ...
  {
    for (i <- 1 to 13) {
      if (c(i)) println(name(i) + " is cyclic");
    }
  }
}
